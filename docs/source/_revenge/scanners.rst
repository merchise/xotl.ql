...
the final byte-code is the same as that of CPython:
  ...
  If the `target`:term: of an `unconditional jump`:term: is a ``RETURN_VALUE``
  replace the jump with a ``RETURN_VALUE``.

Notice, however, this affect the offsets since both `JUMP_FORWARD`:opcode: and
`JUMP_ABSOLUTE`:opcode: have an argument and ``RETURN_VALUE`` does not.  Then
to keep things simple we can modify the rule to pad as many `NOP`:opcode: as
needed:

  If the `target`:term: of an `unconditional jump`:term: is a ``RETURN_VALUE``
  replace the jump with the following instructions::
...
.. note:: In fact we'll find it easier to keep a *single* ``RETURN_VALUE`` at
   the bottom than several spread across the program.  So our scanner actually
   enlarges the CPython version by replacing RETURN_VALUE by JUMPs if the last
   instruction is a RETURN_VALUE.
