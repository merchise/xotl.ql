...
generated byte-code.  The byte-code for basic expression resembles the
...
Now let's proceed our analysis with the boolean expressions.  Boolean
expressions produce byte-code that "short-circuit" the evaluation as soon as
the result is known.  There are not equivalent ``BINARY_*`` byte-code for the
...
\... but they don't.  Furthermore, we'll see that extending our grammar with
something like::
...
won't help because of precedence issues::
