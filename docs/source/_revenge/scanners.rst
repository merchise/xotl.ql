...
Though we have tried to make a linear exposition of the matter the high degree
of coupling between those phases imposes a challenge to justify decisions made
in the first layers to accommodate the actions of the later ones.
...
Since the goal of that package is much more ambitious than ours we have
stripped much of the original grammar and the scanner has been almost entirely
reimplemented.
...
differs in the last step where the walker produces a QST instead.
...
generator expressions and other types of comprehensions and lambdas.  Yield
...
resembles the `postfix notation`_ where operands come before the operator.
...
In fact we'll find it easier to keep *a single RETURN_VALUE* at the bottom
than several spread across the program.  So our scanner actually enlarges the
CPython version by replacing RETURN_VALUE by JUMPs if the last instruction is
a RETURN_VALUE.
